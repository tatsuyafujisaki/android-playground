...
> Composables that accept a Modifier as a parameter to be applied to the whole component represented
> by the composable function should name the parameter modifier and assign the parameter a default
> value of Modifier. It should appear as the first optional parameter in the parameter list
...
- If `size()` is called before `padding()` in a Modifier's method chain, the total size is padding +
  size.
...
- If `background()` is called before `padding()` in a Modifier's method chain, the padding is
  painted.
- If `padding()` is called before `background()` in a Modifier's method chain, the padding is NOT
  painted.
...
- If `clickable()` is called before `padding()` in a Modifier's method chain, the padding is
  clickable.
- If `padding()` is called before `clickable()` in a Modifier's method chain, the padding is NOT
  clickable.
...
- You can keep the scroll position when the Activity is recreated, or even when you scroll away and
  scroll back.
...
- [PagerState.currentPageOffsetFraction](https://developer.android.com/reference/kotlin/androidx/compose/foundation/pager/PagerState#currentPageOffsetFraction())
    - indicates how far the current page is to the snapped position.
        ...
        - never goes beyond -0.5 or 0.5, because if it does, the page is too far from the snapped
          position to be called the current page.
...
> Note: You shouldn't pass ViewModel instances down to other composables. For more information, see
> the Architecture state' holders documentation.
